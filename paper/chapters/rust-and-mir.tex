...
Unlike Java, Rust does not rely on a garbage collector; instead, memory safety is enforced statically at compile time by the \emph{borrow checker}~\cite{rust_book_borrowing}.

\subsection{Ownership and Borrowing}
\label{subsec:ownership-borrowing}

Every value in Rust has a unique \emph{owner}~\cite{rust_book_ownership}.
...
Programs often need temporary access to a value without transferring ownership.
Rust supports this through \emph{borrowing}~\cite{rust_book_borrowing}: a borrow is a reference to a value owned elsewhere.
...
A borrow is only valid for some region of the program, called its \emph{lifetime}~\cite{rust_book_lifetimes}.
...
Rust uses predominantly \emph{structured} control flow: execution is expressed through conditional expressions, pattern matching, and loops rather than arbitrary jumps~\cite{rust_loop_exprs}.
...
The Rust compiler lowers functions into the \emph{Mid-level Intermediate Representation} (MIR) after parsing and earlier compilation stages~\cite{rust_mir_guide}.
...
Once lowered to MIR, execution order is no longer implicit in source nesting but is represented explicitly as a graph.

MIR provides a direct basis for studying the structure of real Rust programs.
It offers a CFG representation suitable for treewidth analysis, and coincides with the level at which the compiler performs its own flow-sensitive analyses~\cite{rust_mir_guide}.
